 ...
 */
export function futuresStatusSubscribe(market: string, onData: () => any) {
 ...
 * @memberof Streams.futures.status
 */
export function futuresStatusUnsubscribe() {
	const client = connections.futures;
	if (!client) throw new Error("Web Socket is not Connected!");
 ...
 */
export async function futuresStatusQuery(market: string, period: number) {
	const client = connections.futures;
	if (!client) throw new Error("Web Socket is not Connected!");
